@extends('layouts.master') @section('header_css')
    ...
    <!--X-editable [ OPTIONAL ]-->
    <link href="{{ asset('assets/plugins/x-editable/css/bootstrap-editable.css') }}"
...
@endsection @section('content')
    ...
    <div class="panel">
        <div class="panel-body">
            ...
            <a href="{{ url('authorized/role/show-trash-role') }}" id="btn-trash" class="btn btn-primary"
               style="margin-left: {{ $margin }}px;">
                <i class="fa fa-trash icon-lg"></i> Thùng rác</a>

            @endpermission
            <table id="roles-admin" class="demo-add-niftycheck" data-toggle="table"
                   data-url="{{ url('/authorized/role/ajax-data-role') }}"
    ...
    <script>
        ...
        function moveTrashItem(item, e) {
            ...
            if (confirm('Xác nhận di chuyển nhóm vào thùng rác ?')) {
                var url = '{{ url("authorized/role/move-trash") }}';
                var data = {
                    '_token': '{{ csrf_token() }}',
                    'id': item
                };
